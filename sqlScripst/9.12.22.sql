	...
	-- -----------------------------------------------------
	-- Table `MC_products`.`Ketegorie`
	-- -----------------------------------------------------
	CREATE TABLE IF NOT EXISTS `MC_products`.`Ketegorie` (
	  `id` INT NOT NULL AUTO_INCREMENT,
	  `nazev` VARCHAR(45) NOT NULL,
	  `dostupne` INT NOT NULL,
	  PRIMARY KEY (`id`))
	...
	CREATE TABLE IF NOT EXISTS `MC_products`.`Produkty` (
	  ...
	  `cena` DOUBLE NOT NULL,
	  `dostupne` INT NOT NULL,
	  `ketegorie` INT NOT NULL,
	  PRIMARY KEY (`íd`),
	  INDEX `fk_Produkty_Ketegorie_idx` (`ketegorie` ASC),
	  CONSTRAINT `fk_Produkty_Ketegorie`
		FOREIGN KEY (`ketegorie`)
		REFERENCES `MC_products`.`Ketegorie` (`id`)
		ON DELETE NO ACTION
		ON UPDATE NO ACTION)
	ENGINE = InnoDB;
